...
class PythonLanguageServer(MethodDispatcher):
    ...
    def m_text_document__did_open(self, textDocument=None, **_kwargs):
        log.info("m_text_document__did_open===========================================================")
        log.info(textDocument)
        workspace = self._match_uri_to_workspace(textDocument['uri'])
        workspace.put_document(textDocument['uri'], textDocument['text'], version=textDocument.get('version'))
        log.info(workspace)
        log.info("put_document success cur_workspace: {}doc_uri:{} source:{}".format(workspace, textDocument['uri'], textDocument['text']))
        self._hook('pyls_document_did_open', textDocument['uri'])
        self.lint(textDocument['uri'], is_saved=True)

    def m_text_document__did_change(self, contentChanges=None, textDocument=None, **_kwargs):
        workspace = self._match_uri_to_workspace(textDocument['uri'])
        log.info("m_text_document__did_change++++++++++++++++++++++ cur_workspace: {}".format(workspace))
        log.info(contentChanges)
        log.info(textDocument['uri'])
        log.info(workspace.get_maybe_document(textDocument['uri']))
